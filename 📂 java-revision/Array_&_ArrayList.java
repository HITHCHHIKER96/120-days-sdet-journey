...
Arrays solve the problem of storing collections of homogeneous data by grouping them under a single data structure.
Arrays must contain elements of the same data type; mixing types is not allowed. For example, an int[] array cannot contain strings or booleans.
The syntax for declaring an array includes specifying the data type followed by square brackets, e.g., int[] rollNumbers;
Initialization involves the new keyword to allocate memory, e.g., rollNumbers = new int[5];
Bullet points:
...
The array reference (int[] arr) is stored in stack memory.
The actual array object (holding the elements) is stored in the Heap.
The new keyword triggers dynamic memory allocation — allocating memory during runtime rather than compile time.
Java’s memory is divided into stack (for reference variables, function call frames) and Heap (for objects including arrays).
Arrays in Java may not be allocated in continuous memory blocks as in languages like C++. This depends on the JVM implementation.
The Java Language Specification (JLS) states that Heap objects are not guaranteed to be contiguous, which means array elements may not be stored in continuous addresses in memory.
Bullet points:
...
Accessing or modifying elements uses syntax like arr[index].
Attempting to access an index outside the declared bounds results in an IndexOutOfBoundsException.
When an array is created, elements of primitive arrays (e.g., int[]) are initialized to default values (e.g., 0 for integers).
Arrays of objects (non-primitives) initialize elements to null because these are references to objects that currently point to no instance.
...
The array in Heap stores reference variables pointing to actual objects, which may be scattered in the Heap.
Each element of the array is a reference pointing to a distinct object.
Until explicitly initialized, these references are null.
This explains why printing an uninitialized String[] shows null values.
Bullet points:
...
Use a for loop to iterate through array indices for input and output.
The array’s .length property provides the size of the array, useful for loop bounds.
Java provides an enhanced for loop (for-each loop) to iterate elements directly, improving code readability.
The standard Arrays.toString() method from the java.util.Arrays class converts arrays to a formatted string representation, simplifying output.
Bullet points:
...
The main array contains references to other arrays (rows).
These sub-arrays can have different lengths, allowing "jagged" arrays.
Only the number of rows is mandatory when declaring a 2D array; the number of columns per row can vary.
Memory allocation for each sub-array happens separately in the Heap.
Accessing an element requires two indices: arr[row][col].
Input and output involve nested loops iterating over rows and columns.
Bullet points:
...
Java passes references by value: a copy of the reference to the array is passed to the function.
Modifications to array elements inside the function affect the original array, as both reference variables point to the same object.
Arrays are mutable, meaning their elements can be changed.
In contrast, Strings are immutable (cannot be changed once created).
This mutability allows in-place operations like swapping or reversing.
Bullet points:
...
Arrays have fixed sizes defined at creation.
When the size is unknown or variable, ArrayList automatically resizes.
Similar to C++ vectors, part of Java’s Collection Framework.
Usage and syntax:
...
Must initialize inner ArrayLists explicitly before adding elements.
Each inner list can have different sizes (similar to jagged multi-dimensional arrays).
Supports dynamic resizing at each level.
Requires nested loops for input/output.
Bullet points:
